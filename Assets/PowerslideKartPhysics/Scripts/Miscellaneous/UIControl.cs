...
namespace PowerslideKartPhysics
{
    ...
    public class UIControl : MonoBehaviour
    {
        ...
        private void Awake() {
            ...
            // Connect mode events to UI updates
            modeController = FindObjectOfType<ModeController>();
            if (modeController != null) {
                modeController.countDownUpdate.AddListener(UpdateCountdownText);
                modeController.modeEndEvent.AddListener(ShowModeSummary);
            }

            if (modeSummary != null) {
                modeSummary.SetActive(false);
            }
        }
        ...
        public void Initialize(Kart kart) {
            ...
            if (targetKart != null) {
                ...
                agent = targetKart.GetComponent<ModeAgent>();
                if (agent != null && agent is RaceAgent raceAgent) {
                    raceAgent.lapCompletedEvent.AddListener(ShowLapTime);
                }
            }
        }

        // Updates the text showing the mode start countdown
        ...
        // Calling this shows all lap times (if applicable) and mode summary at the end of a mode
        public void ShowModeSummary() {
            // Hide the HUD
            if (hudContainer != null) {
                hudContainer.SetActive(false);
            }

            // Show the mode summary UI
            if (modeSummary != null) {
                modeSummary.SetActive(true);
            }

            if (modeController == null) { return; }

            if (modeController is RaceController raceController) {
            ...
            if (kartPositionHolder != null) {
                for (int i = 0; i < modeController.allKarts.Length; i++) {
        ...
        private void Update() {
            ...
            if (agent != null && positionText != null) {
                positionText.text = IntToOrdinal(agent.GetModePosition() + 1);
            }

            if (modeController != null && modeController is RaceController raceController) {
                // Updates the lap count text and race position text of the player kart
                if (agent != null && agent is RaceAgent raceAgent) {
                    if (raceController != null && lapText != null) {
                        lapText.text = raceAgent.lap.ToString() + " / " + raceController.maxLaps.ToString();
                    }
                ...
                if (raceController != null) {
                    ...
                    // Update the kart positions and laps on the mode summary
                    if (modeSummary != null) {
                        if (modeSummary.activeSelf && kartNames.Count == kartLaps.Count && kartLaps.Count == kartPositions.Count) {
                            ModeAgent[] karts = modeController.GetAllKartsSorted();
                            for (int i = 0; i < karts.Length; i++) {
                                kartNames[i].text = karts[i].transform.name;
                                if (karts[i] is RaceAgent ra) {
                                    kartLaps[i].text = Mathf.Min(ra.lapsCompleted, raceController.maxLaps).ToString() + "/" + raceController.maxLaps.ToString();
                                }
                            }
                        }
                    }
